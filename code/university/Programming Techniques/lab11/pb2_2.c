...
{
    int sum = 0;
    if (n_start> n) return 0;
    if (m_start> m) return 0;
    
    if (n==n_start)
    {
        if (m==m_start)
            return array[n][m]%2 == 0 ? array[n][m]: 0;
        else {
    ...
    sum = sum + get_sum(array, n_start, (n+n_start)/2, m_start, (m_start+m)/2);
    sum = sum + get_sum(array, n_start, (n+n_start)/2, (m_start+m)/2+1, m);
    sum = sum + get_sum(array, (n+n_start)/2+1, n, m_start, (m_start+m)/2);
    sum = sum + get_sum(array, (n+n_start)/2+1, n, (m_start+m)/2+1, m);
